...
class FanoutConfiguration {
	...
	@Bean
	RabbitTemplate rabbitTemplateFanout(ConnectionFactory connectionFactory, MessageConverter messageConverter, Exchange fanoutExchange) {
		RabbitTemplate rabbitTemplate = new RabbitTemplate(connectionFactory);
		rabbitTemplate.setMessageConverter(messageConverter);
		rabbitTemplate.setExchange(fanoutExchange.getName());
		rabbitTemplate.setRoutingKey("fanout.message.routing.key");
		return rabbitTemplate;
	}

	@Bean
	FanoutMessagePublisher fanoutMessageSender(RabbitTemplate rabbitTemplateFanout) {
		return new FanoutMessagePublisher(rabbitTemplateFanout);
	}
}
